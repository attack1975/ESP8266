...
static unsigned char ICACHE_FLASH_ATTR read_byte(FILE *file)
{
	...
	//if(GIFLOG) os_printf("Getting data from pos: %d, data: %d, len:%d  - %d - %d - %d \r\n", file->intpos, file->data[file->intpos], file->len, file->data[file->intpos+1], file->data[file->intpos+2], file->data[file->intpos+3]);
...
//		buffer[i++] = '\0';
	for(int i = 0; i < length; i++) {
		if(GIFLOG) os_printf("Getting data from pos: %d, data: %d, len:%d  - %d\r\n", file->intpos, file->data[file->intpos], file->len, length);
...
void * ICACHE_FLASH_ATTR gif_allocz(long bytes)
{
...
GifData * ICACHE_FLASH_ATTR read_gif_data(FILE *file)
{
	GifData *data;
	int size;
	//if(GIFLOG) os_printf("read_gif_data \r\n");
	size = read_byte(file);
...
static ICACHE_FLASH_ATTR unsigned char read_gif_byte(FILE *file, GifDecoder *decoder)
{
	...
	unsigned char next;
	//if(GIFLOG) os_printf("read_gif_byte \r\n");
...
{
	unsigned char *buf = decoder->buf;
	if(GIFLOG) os_printf("finish_gif_picture \r\n");
...
void ICACHE_FLASH_ATTR read_gif_palette(FILE *file, GifPalette *cmap)
{
	int i;
	unsigned char r, g, b;
	if(GIFLOG) os_printf("read_gif_palette \r\n");
...
void ICACHE_FLASH_ATTR print_gif_palette(FILE *file, GifPalette *cmap)
{
	...
	if(GIFLOG) os_printf("  GifPalette (length=%d):\n", cmap->length);
	for (i=0; i<cmap->length; i++) {
		os_printf( "   %02X = 0x", i);
...
{
	unsigned char info;
	if(GIFLOG) os_printf("read_gif_screen \r\n");
...
{
	if(GIFLOG) os_printf("read_gif_extension \r\n");
	GifData *data;
	int i;
...
{
	GifDecoder * deco = gif_allocz(sizeof(GifDecoder));
	if(deco == 0) {
		if(GIFLOG) os_printf("Problems creating new gif decoder, free heap size: %d \r\n", system_get_free_heap_size());
		system_print_meminfo();
		deco = gif_alloc(sizeof(GifDecoder));
	}
	return deco;
}
...
void ICACHE_FLASH_ATTR init_gif_decoder(FILE *file, GifDecoder *decoder)
{
	...
	int lzw_min;
	unsigned int *prefix;
	if(GIFLOG) os_printf("init_gif_decoder: %p \r\n", decoder);
	lzw_min = read_byte(file);

	depth = lzw_min;

	if(GIFLOG) os_printf("init_gif_decoder1: %p \r\n", depth);
	...
	if(GIFLOG) os_printf("init_gif_decoder2: %p \r\n", decoder);
...
int ICACHE_FLASH_ATTR read_gif_code(FILE *file, GifDecoder *decoder) {
	...
	static int code_masks[] = { 0x0000, 0x0001, 0x0003, 0x0007, 0x000f, 0x001f, 0x003f, 0x007f, 0x00ff, 0x01ff, 0x03ff, 0x07ff, 0x0fff };
	...
	while (decoder->shift_state < decoder->running_bits) {
		...
		os_printf("next byte: %d  \r\n", next_byte);
		os_printf ("next byte: "BYTETOBINARYPATTERN, BYTETOBINARY(next_byte));
		decoder->shift_data |= ((unsigned long) next_byte) << decoder->shift_state;
		decoder->shift_state += 8;
	}
	os_printf("decoder->shift_data: %d \r\n", decoder->shift_data);
	os_printf ("decoder->shift_data:: "BYTETOBINARYPATTERN, BYTETOBINARY( decoder->shift_data));

	code = decoder->shift_data & code_masks[decoder->running_bits];
	os_printf("decoder->running_bits: %d \r\n", decoder->running_bits);
	os_printf ("decoder->running_bits:: "BYTETOBINARYPATTERN, BYTETOBINARY( decoder->running_bits));

	os_printf("code: %d \r\n", code);
	os_printf ("code:: "BYTETOBINARYPATTERN, BYTETOBINARY( code));
 ...
 */
void ICACHE_FLASH_ATTR read_gif_line(FILE *file, GifDecoder *decoder, unsigned char *line, int length) {
	int i = 0, j;
			...
			if (current_code < clear_code) {
				...
				if (prefix[current_code] == NO_SUCH_CODE) {
					...
					if (current_code == decoder->running_code - 2) {
						...
						suffix[decoder->running_code - 2] = stack[stack_ptr++] = trace_prefix(prefix, prev_code, clear_code);
					} else {
						return; /* image defect */
					}
				} else
				...
				while (stack_ptr != 0 && i < length) {
					...
					line[i++] = stack[--stack_ptr];
					os_printf("-data[%d]-%d - len = %d\r\n", (i - 1), line[i - 1], length);
				}
			}
			if (prev_code != NO_SUCH_CODE) {
				if ((decoder->running_code < 2) || (decoder->running_code > LZ_MAX_CODE + 2))
					return; /* image defect */
...
static void ICACHE_FLASH_ATTR read_gif_picture_data(FILE *file, GifPicture *pic)
{
	...
	if(decoder == 0) {
		os_printf("ERROR, could not create gif decoder");
		del_gif_decoder(decoder);
		return;
	}

	os_printf("STARTING GETTING REAL DATA");
	...
	del_gif_decoder(decoder);
	if(GIFLOG) os_printf("read_gif_picture_data done \r\n");
}

void ICACHE_FLASH_ATTR read_gif_picture(FILE *file, GifPicture *pic)
{
	unsigned char info;
	if(GIFLOG) os_printf("read_gif_picture \r\n");
	...
	if(GIFLOG) os_printf("read_gif_picture done \r\n");
...
{
	if(GIFLOG) os_printf("read_gif_block \r\n");
	...
	if(GIFLOG) os_printf("read_gif_block done \r\n");
...
//
	...
	while (1) {
		...
		if(GIFLOG) os_printf("block read \r\n");
...
void ICACHE_FLASH_ATTR read_one_gif_picture(FILE *file, Gif *gif)
{
	int i;
	GifBlock *block;
	if(GIFLOG) os_printf("read_one_gif_picture \r\n");
