...
static int sendFile(struct lws * wsi, char * filename1, int to1) {
    ...
    char buf[2000];
        ...
        int address = to1; //strtol(&to1[2], NULL, 16);
...
//            lws_callback_on_writable(tool->wsi);
   ...
   for(int i = 0; i < (sz / 1024); i++) {
     ...
     fclose( f );
     md5addr = 0;
     printf(KRED"REBOOTING TARGET\r\n");

     websocket_write_back(wsi, "FRB: ", 5); // REBOOT
...
static void *pthread_routine(void *tool_in)
{
    ...
    int filesize1 = sendFile(tool->wsi, file, address);
